...
class PermissionController extends Controller
{
    ...
    {
        $permissions = Permission::query()->orderBy('route', 'desc')
            ->paginate(12);
    ...
    public function store(Store $request)
    {
        ...
        $createOrFail = Permission::create([
            'name'  => $name,
            'route' => $route,
        ]);
        // 同步更新权限到超级管理员
        ...
        $createOrFail ? Tool::showMessage('添加成功')
    ...
    public function update(Update $request)
    {
        ...
        $saveOrFail ? Tool::showMessage('修改成功')
    ...
    public function destroy(Request $request)
    {
        ...
        DB::table('model_has_permissions')->whereIn('permission_id', $arr)
            ->delete();
        DB::table('role_has_permissions')->whereIn('permission_id', $arr)
            ->delete();
        $deleteOrFail = Permission::query()->whereIn('id', $arr)->delete();
        $deleteOrFail ? Tool::showMessage('删除成功')
    ...
    public function search(Request $request)
    {
        ...
        $permissions = Permission::query()->where($map)->orderBy('id', 'desc')
            ->paginate(10);
