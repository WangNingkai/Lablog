...
class PushController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $map = [];
        $keyword ? array_push($map, ['subject', 'like', '%'.$keyword.'%'])
            : null;
        $pushes = $this->push
            ->query()
    ...
    public function info($id = null)
    {
        ...
        if (!$response = $this->push->query()
            ->select('subject', 'target', 'content')->find($id)->toArray()
        ) {
    ...
    public function store(Store $request)
    {
        ...
        if (count($target_user) == 1 and $target_user[0] == 1) {
            $target_user = [];
        } // 发送全体
        Tool::pushSubscribe($subject, $content, '', $target_user, $time);
        // 记录日志
        Tool::recordOperation(auth()->user()->name, '推送消息');
        Log::info('Send Message To Subscribe',
            ['to' => $target_user ?: 'all', 'message' => $content]);
