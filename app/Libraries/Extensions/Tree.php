    ...
    public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = '')
    {
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                ...
                $spacer = $adds ? $adds . $j : '';
                $selected = $value[$this->id] == $sid ? 'selected' :
                    '';
                //print_r($value);echo '<hr/>';
                @extract($value);
                $value[$this->pid] == 0 && $str_group ? eval("
               \$nstr = \"$str_group\";") : eval("\$nstr = \"$str\";");
                $this->ret .= $nstr;
                $nbsp = $this->nbsp;
    ...
    public function get_tree_multi($myid, $str, $sid = 0, $adds = '')
    {
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $id => $a) {
                ...
                eval("\$nstr = \"$str\";");
                $this->ret .= $nstr;
                $this->get_tree_multi($id, $str, $sid, $adds .
                    $k . '&nbsp;');
                $number++;
            }
        }
    ...
    function get_treeview($myid = 0, $effected_id = 'example', $str = "<span class='file'>\$name</span>", $str2 = "<span class='folder'>\$name</span>", $showlevel = 0, $style = 'filetree ', $currentlevel = 1, $recursion = false)
    {
        ...
        foreach ($child as $id => $a) {
            ...
            $floder_status = isset($folder) ? ' class="' . $folder
                . '"' : '';
            $this->str .= $recursion ? '<ul><li' . $floder_status .
                ' id=\'' . $id . '\'>' : '<li' . $floder_status . ' id=\'' . $id . '\'>';
            $recursion = false;
            if ($this->get_child($id)) {
                eval("\$nstr = \"$str2\";");
                $this->str .= $nstr;
                if ($showlevel == 0 || ($showlevel > 0 &&
                        $showlevel > $currentlevel)) {
                    $this->get_treeview($id, $effected_id,
                        $str, $str2, $showlevel, $style, $currentlevel + 1, true);
                } elseif ($showlevel > 0 && $showlevel ==
                    $currentlevel) {
