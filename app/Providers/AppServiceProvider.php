...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        view()->composer('*', function ($view) {
            ...
            $category_list = Cache::remember('cache:category_list', 1440,
                function () {
                    // 获取分类导航
            ...
            $top_article_list = Cache::remember('cache:top_article_list', 1440,
                function () {
                    // 获取热门文章
            ...
            $assign = compact('nav_list', 'category_list', 'tag_list',
                'top_article_list', 'link_list', 'config');
            /* @var $view \Illuminate\View\View */
            $view->with($assign);
        });
        # 获取各种统计
        view()->composer('admin.index', function ($view) {
            $articlesCount = Cache::remember('count:article', 1440,
                function () {
                    // 统计文章总数
                    return Article::query()->count('id');
                });

            $commentsCount = Cache::remember('count:comment', 1440,
                function () {
                    // 统计评论总数
                    return Comment::query()->count('id');
                });

            $messagesCount = Cache::remember('count:message', 1440,
                function () {
                    // 统计留言总数
