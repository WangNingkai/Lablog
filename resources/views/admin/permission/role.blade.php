                ...
                <div class="col-md-4">
                    <div class="box">
                        <div class="box-header">
                            ...
                            <form action="{{ route('role_search') }}" method="get" style="display: inline-flex"
                                  class="pull-right">
                                <div class="box-tools">
                            ...
                            <table class="table table-hover">
                                ...
                                @if(!blank($roles))
                                    @foreach($roles as $role)
                                        <tr>
                                            <td><input type="checkbox" value="{{$role->id}}" name="rid"
                                                       class="i-checks"></td>
                                            <td>{{ $role->name }}</td>
                                            <td>
                                    ...
                                    <div class="checkbox">
                                        @foreach( $permissions as $permission)
                                            <label><input type="checkbox" class="i-checks" value="{{$permission->name}}"
                                                          name="permissions[]"
                                                          @if(in_array($permission->name, old('permissions', []))) checked="checked" @endif>&nbsp;{{$permission->name}}
                                            </label>
                                        @endforeach
                                    </div>
                                </div>
