                                    ...
                                    <select class="form-control select2" id="roles" multiple="multiple"
                                        ...
                                        @foreach($roles as $role)
                                            <option value="{{ $role->name }}"
                                                    @if(in_array($role->name,old('roles',[]))) selected @endif>{{ $role->name }}</option>
                                        @endforeach
                                    </select>
                                    @if ($errors->has('roles'))
                                    ...
                                    <div class="radio">
                                        <label class="i-checks">
                                            <input type="radio" name="status" value="{{ \App\Models\User::ACTIVE }}"
                                                   @if(old( 'status', \App\Models\User::FORBID)==\App\Models\User::ACTIVE ) checked="checked" @endif>
                                            &nbsp; 正常
                                        </label>
                                        <label class="i-checks">
                                            <input type="radio" name="status" value="{{ \App\Models\User::FORBID }}"
                                                   @if(old( 'status', \App\Models\User::FORBID)==\App\Models\User::FORBID ) checked="checked" @endif>
                                            &nbsp; 禁用
                                        </label>
                                    </div>
                                </div>
