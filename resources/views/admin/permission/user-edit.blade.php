                                    ...
                                    <select class="form-control select2" id="roles" multiple="multiple"
                                        ...
                                        @foreach($roles as $role)
                                            <option value="{{ $role->name }}"
                                                    @if(!is_null(old('roles')) && in_array($role->name,old('roles',[]))) selected
                                                    @elseif($user->hasRole($role->name)) selected @endif>{{ $role->name }}</option>
                                        @endforeach
                                    </select>
                                    @if ($errors->has('roles'))
                                        <span class="help-block "><strong><i
                                                    class="fa fa-times-circle-o"></i>{{ $errors->first('roles') }}</strong></span>
                                    @endif
                                </div>
                                        ...
                                        <div class="radio">
                                            <label class="i-checks">
                                                <input type="radio" name="status" value="{{ \App\Models\User::ACTIVE }}"
                                                       @if(!is_null(old('status')) && old( 'status') == \App\Models\User::ACTIVE) checked="checked"
                                                       @elseif($user->status == \App\Models\User::ACTIVE ) checked="checked" @endif>
                                                &nbsp; 正常
                                            </label>
                                            <label class="i-checks">
                                                <input type="radio" name="status" value="{{ \App\Models\User::FORBID }}"
                                                       @if(!is_null(old('status')) && old( 'status') == \App\Models\User::FORBID) checked="checked"
                                                       @elseif($user->status == \App\Models\User::FORBID ) checked="checked" @endif>
                                                &nbsp; 禁用
                                            </label>
                                        </div>
                                    </div>
